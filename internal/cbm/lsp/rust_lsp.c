...
static const CBMRegisteredFunc* rust_lookup_method_in_trait(RustLSPContext* ctx,
    const char* trait_qn, const char* method_name);
...
static const char* rust_lookup_type_param_bound(RustLSPContext* ctx,
    const char* name);
static void rust_collect_bounds_from_text(RustLSPContext* ctx, const char* text);
static void rust_record_type_param_bound(RustLSPContext* ctx,
    const char* param_name, const char* trait_qn);
...
void rust_lsp_add_use(RustLSPContext* ctx, const char* local_name, const char* module_path) {
    ...
    if (ctx->use_count % 32 == 0) {
        ...
        const char** nl = (const char**)cbm_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(char*));
        const char** np = (const char**)cbm_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(char*));
        if (!nl || !np) {
            return;
        }
...
void rust_lsp_add_glob(RustLSPContext* ctx, const char* module_qn) {
    ...
    if (ctx->glob_count % 16 == 0) {
        ...
        const char** ng = (const char**)cbm_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(char*));
        if (!ng) {
            return;
        }
...
static const char* RUST_PRIMITIVES[] = {
...
static bool is_void_macro(const char* name) {
    ...
    static const char* m[] = {"println", "print", "eprintln", "eprint", "panic",
        "unimplemented", "todo", "unreachable", "assert", "assert_eq", "assert_ne",
        "debug_assert", "debug_assert_eq", "debug_assert_ne", "writeln", "write",
        NULL};
...
static bool is_prelude_trait_method(const char* method_name) {
    ...
    static const char* m[] = {
        ...
        "next", "iter", "iter_mut", "into_iter",
        "map", "filter", "fold", "for_each", "collect", "count", "sum",
        "max", "min", "any", "all", "find", "position", "enumerate",
        "zip", "chain", "take", "skip", "rev", "cloned", "copied",
        "by_ref", "step_by", "flat_map", "flatten", "filter_map", "peekable",
        /* Future */
        "poll",
        NULL
    };
...
typedef struct { const char* name; const char* qn; } RustPreludeEntry;

static const RustPreludeEntry RUST_PRELUDE[] = {
...
static const char* rust_lookup_prelude(const char* name) {
    ...
    for (const RustPreludeEntry* e = RUST_PRELUDE; e->name; e++) {
        if (strcmp(e->name, name) == 0) return e->qn;
    }
    return NULL;
}
...
static const char* rust_resolve_path_expr(RustLSPContext* ctx, const char* path) {
    ...
    if (strncmp(path, "Self::", 6) == 0 && ctx->self_type_qn) {
        return cbm_arena_sprintf(ctx->arena, "%s.%s",
            ctx->self_type_qn, convert_path_to_qn(ctx->arena, path + 6));
    }
    ...
    if (strncmp(path, "crate::", 7) == 0 && ctx->module_qn) {
        ...
        size_t crate_len = second_dot ? (size_t)(second_dot - ctx->module_qn)
                                      : strlen(ctx->module_qn);
        char* crate_buf = cbm_arena_strndup(ctx->arena, ctx->module_qn, crate_len);
        return cbm_arena_sprintf(ctx->arena, "%s.%s",
            crate_buf, convert_path_to_qn(ctx->arena, path + 7));
    }
    ...
    if (strncmp(path, "super::", 7) == 0 && ctx->module_qn) {
        ...
        if (dot) {
            char* parent = cbm_arena_strndup(ctx->arena, ctx->module_qn,
                                             (size_t)(dot - ctx->module_qn));
            return cbm_arena_sprintf(ctx->arena, "%s.%s",
                parent, convert_path_to_qn(ctx->arena, path + 7));
        }
    }
    ...
    if (ctx->cargo_manifest) {
        const CBMCargoManifest* m =
            (const CBMCargoManifest*)ctx->cargo_manifest;
        ...
        if (mem) {
            ...
            return cbm_arena_sprintf(ctx->arena, "%s.%s",
                head, convert_path_to_qn(ctx->arena, tail));
        }
...
static const CBMType* parse_type_text_with_params(CBMArena* arena, const char* text,
    ...
    if (text[0] == '*') {
        ...
        return cbm_type_pointer(arena, parse_type_text_with_params(arena, p, module_qn, type_params));
    }
    ...
    if (text[0] == '[' && text[strlen(text) - 1] == ']') {
        ...
        return cbm_type_slice(arena, parse_type_text_with_params(arena, inner, module_qn, type_params));
    }
        ...
        for (char* q = inner; ; q++) {
            ...
            if ((*q == ',' && depth == 0) || *q == '\0') {
                ...
                *q = '\0';
                ...
                while (*start == ' ') start++;
                if (count < 15 && *start) {
                    elems[count++] = parse_type_text_with_params(arena, start, module_qn, type_params);
                }
                if (save == '\0') break;
                start = q + 1;
            }
        }
        if (count == 0) return cbm_type_builtin(arena, "()");
                ...
                if ((*q == ',' && depth == 0) || *q == '\0') {
                    ...
                    *q = '\0';
                    ...
                    if (*start != '\'' && *start && targ_count < 15) {
                        targs[targ_count++] = parse_type_text_with_params(arena, start, module_qn, type_params);
                    }
                    if (save == '\0') break;
                    start = q + 1;
                }
 ...
 * types. */
static const CBMType* rust_substitute_type(CBMArena* arena, const CBMType* t,
    const char** params, const CBMType** args) {
    if (!t || !params || !args) {
        return t;
    }
    switch (t->kind) {
    ...
    case CBM_TYPE_REFERENCE:
        return cbm_type_reference(arena, rust_substitute_type(arena, t->data.reference.elem, params, args));
    case CBM_TYPE_POINTER:
        return cbm_type_pointer(arena, rust_substitute_type(arena, t->data.pointer.elem, params, args));
    case CBM_TYPE_SLICE:
    ...
    case CBM_TYPE_TEMPLATE: {
        ...
        for (; n < t->data.template_type.arg_count && n < 16; n++) {
            new_args[n] = rust_substitute_type(arena, t->data.template_type.template_args[n],
                params, args);
        }
...
static void rust_unify_type(const CBMType* param_type, const CBMType* arg_type,
    ...
    if (param_type->kind == CBM_TYPE_REFERENCE) {
        rust_unify_type(param_type->data.reference.elem, arg_type, type_param_names, inferred, param_count);
        return;
    }
    if (param_type->kind == CBM_TYPE_SLICE && arg_type->kind == CBM_TYPE_SLICE) {
        rust_unify_type(param_type->data.slice.elem, arg_type->data.slice.elem,
            type_param_names, inferred, param_count);
        return;
    }
        ...
        if (param_type->data.template_type.arg_count == arg_type->data.template_type.arg_count) {
            ...
            for (int i = 0; i < ac; i++) {
                rust_unify_type(param_type->data.template_type.template_args[i],
                           arg_type->data.template_type.template_args[i],
                    type_param_names, inferred, param_count);
            }
        }
        return;
    ...
    if (param_type->kind == CBM_TYPE_TUPLE && arg_type->kind == CBM_TYPE_TUPLE) {
        ...
        for (int i = 0; i < min_; i++) {
            rust_unify_type(param_type->data.tuple.elems[i],
                arg_type->data.tuple.elems[i],
                type_param_names, inferred, param_count);
        }
        return;
    }
...
static const CBMType* rust_apply_subst(CBMArena* arena, const CBMType* t,
    const char** names, const CBMType** inferred, int count) {
    if (!t) return t;
    switch (t->kind) {
    case CBM_TYPE_TYPE_PARAM:
        for (int i = 0; i < count; i++) {
            if (inferred[i] && names[i] &&
                strcmp(t->data.type_param.name, names[i]) == 0) {
                return inferred[i];
            }
        }
        return t;
    case CBM_TYPE_REFERENCE:
        return cbm_type_reference(arena,
            rust_apply_subst(arena, t->data.reference.elem, names, inferred, count));
    case CBM_TYPE_POINTER:
        return cbm_type_pointer(arena,
            rust_apply_subst(arena, t->data.pointer.elem, names, inferred, count));
    case CBM_TYPE_SLICE:
    ...
    case CBM_TYPE_TEMPLATE: {
        ...
        if (n > 16) n = 16;
        for (int i = 0; i < n; i++) {
            new_args[i] = rust_apply_subst(arena,
                t->data.template_type.template_args[i], names, inferred, count);
        }
        return cbm_type_template(arena, t->data.template_type.template_name,
            new_args, n);
    }
    case CBM_TYPE_TUPLE: {
        ...
        if (n > 16) n = 16;
        for (int i = 0; i < n; i++) {
            new_elems[i] = rust_apply_subst(arena, t->data.tuple.elems[i],
                names, inferred, count);
        }
        return cbm_type_tuple(arena, new_elems, n);
    }
    default:
...
const CBMType* rust_eval_expr_type(RustLSPContext* ctx, TSNode node) {
                ...
                if (qn) {
                    ...
                    if (f && f->signature && f->signature->kind == CBM_TYPE_FUNC) {
                        ...
                        if (rt_arr && rt_arr[0]) {
                            ...
                            if (f->receiver_type && cbm_type_is_unknown(ret) &&
                                f->short_name &&
                                ...
                                if (is_wrapper && !ts_node_is_null(args_node)) {
                                    ...
                                    if (anc > 0) {
                                        const CBMType* arg_t = rust_eval_expr_type(ctx,
                                            ts_node_named_child(args_node, 0));
                                        if (arg_t && !cbm_type_is_unknown(arg_t)) {
                                            return cbm_type_template(ctx->arena,
                                                f->receiver_type, &arg_t, 1);
                                        }
                                    }
                                }
                    ...
                    if (dot) {
                        ...
                        const CBMRegisteredFunc* m = cbm_registry_lookup_method_aliased(ctx->registry,
                            head, short_name);
                        if (!m && ctx->module_qn) {
                            const char* full_head = cbm_arena_sprintf(ctx->arena, "%s.%s",
                                ctx->module_qn, head);
                            m = cbm_registry_lookup_method_aliased(ctx->registry,
                                full_head, short_name);
                            if (m) head = (char*)full_head;
                        }
        ...
        if (strcmp(fk, "field_expression") == 0) {
            ...
            if (!ts_node_is_null(value) && !ts_node_is_null(field)) {
                ...
                if (mname && recv) {
                    ...
                    if (base && base->kind == CBM_TYPE_TEMPLATE) {
                        ...
                        if (strstr(tname, "Vec") || strstr(tname, "VecDeque") ||
                            ...
                            if (strcmp(mname, "iter") == 0 || strcmp(mname, "iter_mut") == 0 ||
                                strcmp(mname, "into_iter") == 0 ||
                                strcmp(mname, "drain") == 0) {
                        ...
                        if (strstr(tname, "Option")) {
                            ...
                            if (strcmp(mname, "as_ref") == 0) {
                                if (base->data.template_type.arg_count > 0) {
                                    const CBMType* arg0 = cbm_type_reference(ctx->arena,
                                        base->data.template_type.template_args[0]);
                                    return cbm_type_template(ctx->arena, "core.option.Option",
                                        &arg0, 1);
                                }
                        ...
                        if (strstr(tname, "Iterator")) {
                            ...
                            if (strcmp(mname, "next") == 0 ||
                                ...
                                strcmp(mname, "max") == 0 ||
                                strcmp(mname, "min") == 0 ||
                                strcmp(mname, "max_by") == 0 ||
                                strcmp(mname, "min_by") == 0) {
                                if (base->data.template_type.arg_count > 0) {
                                    const CBMType* a0 = base->data.template_type.template_args[0];
                                    return cbm_type_template(ctx->arena, "core.option.Option", &a0, 1);
                                }
                            ...
                            if (strcmp(mname, "filter") == 0 || strcmp(mname, "take") == 0 ||
                                ...
                                strcmp(mname, "take_while") == 0 || strcmp(mname, "skip_while") == 0 ||
                                strcmp(mname, "inspect") == 0) {
                                return base;  /* preserves Iterator<Item> */
                            }
                            if ((strcmp(mname, "sum") == 0 ||
                                 strcmp(mname, "product") == 0 ||
                                 strcmp(mname, "fold") == 0 ||
                                 strcmp(mname, "reduce") == 0) &&
                            ...
                            if (strcmp(mname, "position") == 0) {
                                ...
                                return cbm_type_template(ctx->arena, "core.option.Option", &usize, 1);
                            }
                        ...
                        if (strstr(tname, "HashMap") || strstr(tname, "BTreeMap")) {
                            ...
                            if (strcmp(mname, "get") == 0 ||
                                strcmp(mname, "get_mut") == 0 ||
                                strcmp(mname, "remove") == 0) {
                                if (base->data.template_type.arg_count > 1) {
                                    const CBMType* v = base->data.template_type.template_args[1];
                                    return cbm_type_template(ctx->arena, "core.option.Option", &v, 1);
                                }
                            }
                            if (strcmp(mname, "iter") == 0 ||
                                strcmp(mname, "iter_mut") == 0) {
                                /* Iterator<(K, V)>. */
                                if (base->data.template_type.arg_count > 1) {
                                    const CBMType* pair[2] = {
                                        base->data.template_type.template_args[0],
                            ...
                            if (strcmp(mname, "values") == 0 &&
                                ...
                                return cbm_type_template(ctx->arena, "core.iter.Iterator",
                                    &base->data.template_type.template_args[1], 1);
                            }
                        }
                    }
                    ...
                    if (t && t->kind == CBM_TYPE_FUNC &&
                        t->data.func.return_types &&
                        ...
                        if (ret->kind == CBM_TYPE_NAMED &&
                            ...
                            while (rb && rb->kind == CBM_TYPE_REFERENCE) rb = rb->data.reference.elem;
    ...
    if (strcmp(kind, "unary_expression") == 0) {
        ...
        const CBMType* inner = ts_node_is_null(operand)
            ? cbm_type_unknown() : rust_eval_expr_type(ctx, operand);
        if (op && strcmp(op, "*") == 0) {
    ...
    if (strcmp(kind, "index_expression") == 0) {
        ...
        while (base && base->kind == CBM_TYPE_REFERENCE) base = base->data.reference.elem;
        if (base && base->kind == CBM_TYPE_SLICE) {
            return base->data.slice.elem;
        }
        if (base && base->kind == CBM_TYPE_TEMPLATE) {
            const char* nm = base->data.template_type.template_name;
            if ((strstr(nm, "Vec") || strstr(nm, "VecDeque")) && base->data.template_type.arg_count > 0) {
                return base->data.template_type.template_args[0];
            }
            if ((strstr(nm, "HashMap") || strstr(nm, "BTreeMap")) && base->data.template_type.arg_count > 1) {
    ...
    if (strcmp(kind, "block") == 0) {
        ...
        for (uint32_t i = nc; i > 0; i--) {
            ...
            if (strcmp(ck, "expression_statement") != 0 &&
                strcmp(ck, "let_declaration") != 0 &&
                strcmp(ck, "empty_statement") != 0 &&
                strcmp(ck, "line_comment") != 0 &&
...
const CBMType* rust_eval_expr_typed(RustLSPContext* ctx, TSNode node,
    const CBMType* expected) {
    const CBMType* synth = rust_eval_expr_type(ctx, node);
    if (!expected) return synth;
    ...
    if (synth->kind == CBM_TYPE_TEMPLATE && expected->kind == CBM_TYPE_TEMPLATE &&
        synth->data.template_type.template_name &&
        expected->data.template_type.template_name &&
        strcmp(synth->data.template_type.template_name,
               expected->data.template_type.template_name) == 0 &&
        synth->data.template_type.arg_count == 0 &&
        expected->data.template_type.arg_count > 0) {
        return expected;
    }
    /* NAMED matches expected NAMED — no refinement needed. */
    if (synth->kind == CBM_TYPE_NAMED && expected->kind == CBM_TYPE_TEMPLATE &&
        synth->data.named.qualified_name &&
        expected->data.template_type.template_name &&
        strcmp(synth->data.named.qualified_name,
               expected->data.template_type.template_name) == 0) {
...
static const CBMType* rust_eval_member_access(RustLSPContext* ctx, const CBMType* recv,
    const char* member) {
    if (!recv || !member) return cbm_type_unknown();
    ...
    if (base->kind == CBM_TYPE_NAMED) {
        ...
        if (strcmp(nm, "str") == 0) type_qn = "core.str";
        else if (strcmp(nm, "String") == 0) type_qn = "alloc.string.String";
        else type_qn = nm;
    ...
    if (method && method->signature) {
        if (template_args && method->type_param_names) {
            ...
            const CBMType* sub = rust_substitute_type(ctx->arena, method->signature,
...
static const CBMType* rust_lookup_field(RustLSPContext* ctx,
    const char* type_qn, const char* field_name, int depth) {
    if (!type_qn || !field_name || depth > CBM_LSP_MAX_LOOKUP_DEPTH) return NULL;
    const CBMRegisteredType* rt = cbm_registry_lookup_type(ctx->registry, type_qn);
    if (!rt) return NULL;
    if (rt->alias_of) return rust_lookup_field(ctx, rt->alias_of, field_name, depth + 1);
    if (rt->field_names) {
        for (int i = 0; rt->field_names[i]; i++) {
            if (strcmp(rt->field_names[i], field_name) == 0 &&
                rt->field_types && rt->field_types[i]) {
...
static bool rust_type_derefs_to_first_arg(const char* type_qn) {
    if (!type_qn) return false;
    static const char* derefable[] = {
        "alloc.boxed.Box",        "std.boxed.Box",
        "alloc.rc.Rc",            "std.rc.Rc",
        "alloc.sync.Arc",         "std.sync.Arc",
        "core.cell.RefCell",      "std.cell.RefCell",
    ...
    for (const char** p = derefable; *p; p++) {
        if (strcmp(*p, type_qn) == 0) return true;
    }
    return false;
}
...
static const CBMRegisteredFunc* rust_lookup_method_depth(RustLSPContext* ctx,
    ...
    const CBMRegisteredFunc* f = cbm_registry_lookup_method_aliased(ctx->registry, type_qn, member_name);
    if (f) return f;
    ...
    if (rt) {
        ...
        if (rt->embedded_types) {
            for (int i = 0; rt->embedded_types[i]; i++) {
                ...
                if (strncmp(rt->embedded_types[i], "DerefTarget:", 12) == 0) continue;
                ...
                if (strncmp(rt->embedded_types[i], "Bound:", 6) == 0) {
                    ...
                    f = rust_lookup_method_depth(ctx, rt->embedded_types[i] + 6,
                        member_name, depth + 1);
                    if (f) return f;
                    continue;
                }
...
static const CBMRegisteredFunc* rust_lookup_method_in_trait(RustLSPContext* ctx,
    const char* trait_qn, const char* method_name) {
    if (!ctx || !trait_qn || !method_name) return NULL;
...
static const CBMRegisteredFunc* rust_resolve_trait_method(RustLSPContext* ctx,
    const char* receiver_type_qn, const char* method_name, int* out_impl_count) {
    if (out_impl_count) *out_impl_count = 0;
    if (!ctx || !receiver_type_qn || !method_name) return NULL;
    const CBMTypeRegistry* reg = ctx->registry;
    ...
    const CBMRegisteredFunc* inh = cbm_registry_lookup_method_aliased(reg, receiver_type_qn, method_name);
    if (inh) {
        if (out_impl_count) *out_impl_count = 1;
        return inh;
    }
    ...
    for (int ti = 0; ti < reg->type_count && impls < 3; ti++) {
        ...
        if (!t->embedded_types) continue;
        for (int j = 0; t->embedded_types[j]; j++) {
            if (strcmp(t->embedded_types[j], receiver_type_qn) == 0) {
                const CBMRegisteredFunc* mf = cbm_registry_lookup_method(reg,
                    t->qualified_name, method_name);
                if (mf) {
                    impls++;
                    if (impls == 1) unique = mf;
                }
                break;
            }
        }
    }
    if (out_impl_count) *out_impl_count = impls;
    if (impls == 1) return unique;
...
static void rust_walk_macro_tokens(RustLSPContext* ctx, TSNode node) {
    ...
    const char* kind = ts_node_type(node);
    if (strcmp(kind, "call_expression") == 0 ||
        strcmp(kind, "macro_invocation") == 0 ||
    ...
    for (uint32_t i = 0; i < nc; i++) {
        TSNode c = ts_node_child(node, i);
        if (!ts_node_is_null(c)) rust_walk_macro_tokens(ctx, c);
    }
}
...
static void rust_macro_strip_outer(const char* tt, int len,
...
static void rust_record_macro_rule(RustLSPContext* ctx, const char* macro_name,
    TSNode pattern, TSNode transcriber) {
    if (!ctx || !macro_name || ts_node_is_null(transcriber)) return;
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(k, "macro_definition") == 0) {
            ...
            for (uint32_t j = 0; j < mnc; j++) {
                ...
                if (ts_node_is_null(rule) || !ts_node_is_named(rule)) continue;
                if (strcmp(ts_node_type(rule), "macro_rule") != 0) continue;
            ...
            if (!ts_node_is_null(body)) rust_collect_macro_rules(ctx, body);
        }
    }
}
...
static int macro_consume_ident(const char* s, int len, int from) {
    int p = from;
    if (p >= len) return from;
    char c = s[p];
    if (!(c == '_' || (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'))) return from;
    p++;
    while (p < len) {
        c = s[p];
...
static int macro_consume_balanced(const char* s, int len, int from) {
    ...
    while (p < len && depth > 0) {
        ...
        if (c == '\'') {
            ...
            int q = p + 1;
            if (q < len && s[q] == '\\') { q++; if (q < len) q++; }
            else if (q < len) q++;
            if (q < len && s[q] == '\'') { p = q + 1; continue; }
...
static int macro_consume_fragment(const char* s, int len, int from,
    const char* frag) {
    ...
    if (strcmp(frag, "literal") == 0) {
        ...
        char c = s[from];
        if (c == '"') return macro_consume_balanced(s, len, from);
        if (c == '\'') {
            int q = from + 1;
            if (q < len && s[q] == '\\') { q++; if (q < len) q++; }
            else if (q < len) q++;
            if (q < len && s[q] == '\'') return q + 1;
            return from;
        }
        if (c == '-' || (c >= '0' && c <= '9')) {
            int p = from;
            if (c == '-') p++;
            while (p < len && ((s[p] >= '0' && s[p] <= '9') || s[p] == '_' ||
                               s[p] == '.' || s[p] == 'x' || s[p] == 'X' ||
                               s[p] == 'b' || s[p] == 'o' ||
                               (s[p] >= 'a' && s[p] <= 'f') ||
                               (s[p] >= 'A' && s[p] <= 'F'))) p++;
    ...
    if (strcmp(frag, "block") == 0) {
        if (s[from] == '{') return macro_consume_balanced(s, len, from);
        return from;
    }
 ...
 * name doesn't fit. */
static bool macro_env_bind(MacroEnv* env, const char* name, int name_len,
    const char* val, int val_len) {
    if (env->count >= RUST_MACRO_MAX_BINDINGS) return false;
...
static const MacroBinding* macro_env_lookup(const MacroEnv* env, const char* name) {
    for (int i = 0; i < env->count; i++) {
        if (strcmp(env->kv[i].name, name) == 0) return &env->kv[i];
    }
    return NULL;
}
...
static bool macro_pattern_match(const char* pat, int pat_len,
    const char* in, int in_len, MacroEnv* env) {
        ...
        if (pc == '$') {
            ...
            if (pp < pat_len && pat[pp] == '(') {
                ...
                if (next_lit >= pat_len) {
                    ...
                    while (ip < in_len && in[ip] != target) ip++;
                }
                continue;
            }
            /* Metavar name. */
            ...
            ip = val_end;
            if (!macro_env_bind(env, pat + name_start, name_end - name_start,
                                in + val_start, val_end - val_start)) {
                return false;
            }
            continue;
        }
...
static char* macro_substitute(CBMArena* arena,
    const char* xs, int xs_len, const MacroEnv* env) {
    ...
    while (xp < xs_len) {
        ...
        if (c == '$' && xp + 1 < xs_len) {
            ...
            if (xs[xp + 1] == '(') {
                ...
                char* inner = macro_substitute(arena,
                    xs + body_start, body_end - body_start, env);
                if (inner) {
...
static void rust_expand_user_macro(RustLSPContext* ctx, const char* mname,
    TSNode invocation) {
    if (!ctx || !mname || !ctx->macro_rules_arr) return;
    ...
    for (int i = 0; i < ctx->macro_rules_count; i++) {
        ...
        if (strcmp(r->macro_name, mname) != 0) continue;
        memset(&env, 0, sizeof(env));
        if (r->pattern_text && inv_args &&
            macro_pattern_match(r->pattern_text, r->pattern_len,
                                inv_args, inv_args_len, &env)) {
            hit = r;
            break;
        }
    ...
    if (!hit || !hit->transcriber_text || hit->transcriber_len <= 0) return;

    /* Substitute the bound metavars into the transcriber body. */
    char* substituted = macro_substitute(ctx->arena,
        hit->transcriber_text, hit->transcriber_len, &env);
    if (!substituted) return;

    /* Wrap and parse. */
    char* wrapped = cbm_arena_sprintf(ctx->arena,
        "fn __cbm_macro_expand() { %s; }\n", substituted);
    if (!wrapped) return;
...
static void rust_bind_pattern(RustLSPContext* ctx, TSNode pattern, const CBMType* type) {
    ...
    if (strcmp(kind, "captured_pattern") == 0) {
        ...
        if (!ts_node_is_null(sub)) rust_bind_pattern(ctx, sub, type);
        return;
    }
    ...
    if (strcmp(kind, "struct_pattern") == 0) {
        ...
        while (base && base->kind == CBM_TYPE_REFERENCE) base = base->data.reference.elem;
        const char* type_qn = NULL;
        if (base && base->kind == CBM_TYPE_NAMED) type_qn = base->data.named.qualified_name;
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(fk, "field_pattern") == 0) {
                ...
                if (!fname) continue;
                const CBMType* ft = type_qn ? rust_lookup_field(ctx, type_qn, fname, 0) : cbm_type_unknown();
                if (!ft) ft = cbm_type_unknown();
...
void rust_process_statement(RustLSPContext* ctx, TSNode node) {
    ...
    if (strcmp(kind, "let_declaration") == 0) {
        ...
        if (!let_type) let_type = cbm_type_unknown();
        if (!ts_node_is_null(pat)) rust_bind_pattern(ctx, pat, let_type);
        return;
    }
    ...
    if (strcmp(kind, "const_item") == 0 || strcmp(kind, "static_item") == 0) {
        TSNode name_node = ts_node_child_by_field_name(node, "name", 4);
        TSNode tn = ts_node_child_by_field_name(node, "type", 4);
        const CBMType* type = ts_node_is_null(tn) ? cbm_type_unknown()
                                                  : rust_parse_type_node(ctx, tn);
        if (!ts_node_is_null(name_node)) {
            char* name = rust_node_text(ctx, name_node);
            if (name) cbm_scope_bind(ctx->current_scope, name, type);
        }
        return;
    }
}
...
static void rust_resolve_call_expression(RustLSPContext* ctx, TSNode node) {
    ...
    if (strcmp(fk, "field_expression") == 0) {
        TSNode value = ts_node_child_by_field_name(func_node, "value", 5);
        TSNode field = ts_node_child_by_field_name(func_node, "field", 5);
        if (ts_node_is_null(value) || ts_node_is_null(field)) return;
        char* mname = rust_node_text(ctx, field);
        if (!mname) return;
        ...
        const char* type_qn = NULL;
        if (base && base->kind == CBM_TYPE_NAMED) type_qn = base->data.named.qualified_name;
        else if (base && base->kind == CBM_TYPE_TEMPLATE) type_qn = base->data.template_type.template_name;
        ...
        if (type_qn) {
            int impl_count = 0;
            const CBMRegisteredFunc* m = rust_resolve_trait_method(ctx, type_qn, mname, &impl_count);
            if (m) {
            ...
            for (int hop = 0; hop < 8; hop++) {
                ...
                while (next && (next->kind == CBM_TYPE_REFERENCE || next->kind == CBM_TYPE_POINTER)) {
                    next = (next->kind == CBM_TYPE_REFERENCE)
                        ? next->data.reference.elem : next->data.pointer.elem;
                }
                if (!next) break;
                const char* next_qn = NULL;
                if (next->kind == CBM_TYPE_NAMED) next_qn = next->data.named.qualified_name;
                else if (next->kind == CBM_TYPE_TEMPLATE) next_qn = next->data.template_type.template_name;
                else if (next->kind == CBM_TYPE_BUILTIN) {
                    const char* nm = next->data.builtin.name;
                    if (strcmp(nm, "str") == 0) next_qn = "core.str";
                    else if (strcmp(nm, "String") == 0) next_qn = "alloc.string.String";
                }
                if (!next_qn) break;
                int hop_impls = 0;
                const CBMRegisteredFunc* hm = rust_resolve_trait_method(ctx, next_qn, mname, &hop_impls);
                if (hm) {
                    rust_emit_resolved_call(ctx, hm->qualified_name,
                        "lsp_deref_dispatch", CBM_RUST_CONF_PROMOTED);
                    return;
                }
                cur = next;
            }
             ...
             * the bound trait. */
            {
                ...
                if (dot) short_qn = dot + 1;
                const char* bound = rust_lookup_type_param_bound(ctx, short_qn);
                if (bound) {
                    int bimpls = 0;
                    const CBMRegisteredFunc* bm = rust_resolve_trait_method(
                        ctx, bound, mname, &bimpls);
                    if (bm) {
                        rust_emit_resolved_call(ctx, bm->qualified_name,
                            "lsp_bound_dispatch", CBM_RUST_CONF_TRAIT_AMB);
                        return;
                    }
                }
            }
    ...
    if (strcmp(fk, "identifier") == 0 || strcmp(fk, "scoped_identifier") == 0 ||
        ...
        if (strcmp(fk, "generic_function") == 0) {
            ...
            if (!ts_node_is_null(inner)) actual_func = inner;
        }
        ...
        if (dot) {
            ...
            if (m) {
                rust_emit_resolved_call(ctx, m->qualified_name,
                    strcmp(short_name, "new") == 0 ? "lsp_constructor" : "lsp_ufcs",
                    CBM_RUST_CONF_UFCS);
                return;
            }
            ...
            if (tm) {
                rust_emit_resolved_call(ctx, tm->qualified_name,
                    impls == 1 ? "lsp_trait_ufcs" : "lsp_trait_ufcs_amb",
        ...
        if (tail && *tail && ctx->module_qn) {
            ...
            size_t crate_len = first_dot ? (size_t)(first_dot - ctx->module_qn)
                                         : strlen(ctx->module_qn);
            const CBMRegisteredFunc* unique = NULL;
            int matches = 0;
            for (int i = 0; i < ctx->registry->func_count && matches < 2; i++) {
                const CBMRegisteredFunc* f = &ctx->registry->funcs[i];
                if (!f->short_name || !f->qualified_name) continue;
                if (f->receiver_type) continue;  /* free functions only */
                if (strcmp(f->short_name, tail) != 0) continue;
                /* Crate-scoped: QN must start with the same prefix. */
                if (strncmp(f->qualified_name, ctx->module_qn, crate_len) != 0) continue;
                matches++;
                if (matches == 1) unique = f;
            }
            if (matches == 1 && unique) {
                rust_emit_resolved_call(ctx, unique->qualified_name,
                    "lsp_short_name_unique", CBM_RUST_CONF_PROMOTED);
                return;
            }
        }
...
static void rust_resolve_calls_in_node(RustLSPContext* ctx, TSNode node) {
    ...
    if (ctx->eval_step_count > CBM_RUST_EVAL_STEP_CAP) return;
    ctx->eval_step_count++;
    const char* kind = ts_node_type(node);
        ...
        if (!ts_node_is_null(fn) && strcmp(ts_node_type(fn), "field_expression") == 0) {
            ...
            if (!ts_node_is_null(val) && !ts_node_is_null(fld)) {
                ...
                static const char* item_methods[] = {
                    ...
                    "take_while", "skip_while", "filter_map", "inspect",
                    "max_by", "min_by", "max_by_key", "min_by_key", "sort_by_key",
                    NULL
                };
                bool is_item_method = false;
                if (mname) {
                    for (const char** mm = item_methods; *mm; mm++) {
                        if (strcmp(mname, *mm) == 0) { is_item_method = true; break; }
                    }
                }
                if (is_item_method) {
    ...
    if (strcmp(kind, "macro_invocation") == 0) {
        ...
        if (!ts_node_is_null(mname_node)) {
            ...
            if (mname) {
                ...
                } else if (strcmp(mname, "env") == 0 ||
                           strcmp(mname, "option_env") == 0) {
        ...
        TSNode args = ts_node_child_by_field_name(node, "arguments", 9);
        if (!ts_node_is_null(args)) rust_walk_macro_tokens(ctx, args);
    ...
    bool push_scope = (strcmp(kind, "block") == 0 ||
                       strcmp(kind, "if_expression") == 0 ||
                       strcmp(kind, "if_let_expression") == 0 ||
                       strcmp(kind, "while_expression") == 0 ||
                       strcmp(kind, "while_let_expression") == 0 ||
                       strcmp(kind, "for_expression") == 0 ||
                       strcmp(kind, "match_arm") == 0 ||
                       strcmp(kind, "closure_expression") == 0);
        ...
        if (!ts_node_is_null(pat) && !ts_node_is_null(val)) {
            ...
            if (base && base->kind == CBM_TYPE_SLICE) {
                ...
                if ((strstr(nm, "Vec") || strstr(nm, "VecDeque") || strstr(nm, "Iterator") ||
                     strstr(nm, "Range")) && base->data.template_type.arg_count > 0) {
...
static void rust_process_function(RustLSPContext* ctx, TSNode func_node, const char* parent_qn) {
    ...
    if (!ts_node_is_null(tp_list)) {
        ...
        if (tp_text) rust_collect_bounds_from_text(ctx, tp_text);
    }
    ...
    if (!ts_node_is_null(where_clause)) {
        ...
        if (wt) rust_collect_bounds_from_text(ctx, wt);
    }
    ...
    if (!ts_node_is_null(params)) {
        ...
        for (uint32_t i = 0; i < pc; i++) {
            ...
            if (strcmp(pk, "parameter") == 0) {
                ...
                TSNode tn = ts_node_child_by_field_name(p, "type", 4);
                const CBMType* pt = ts_node_is_null(tn) ? cbm_type_unknown()
                                                        : rust_parse_type_node(ctx, tn);
                if (!ts_node_is_null(pat)) rust_bind_pattern(ctx, pat, pt);
            }
        }
    }
...
static void rust_record_type_param_bound(RustLSPContext* ctx,
    const char* param_name, const char* trait_qn) {
    if (!ctx || !param_name || !trait_qn) return;
    /* Grow by 8s. */
    if (ctx->type_param_bound_count % 8 == 0) {
        int new_cap = ctx->type_param_bound_count + 8;
        void* narr = cbm_arena_alloc(ctx->arena,
            new_cap * sizeof(*ctx->type_param_bounds));
        if (!narr) return;
...
static void rust_collect_bounds_from_text(RustLSPContext* ctx, const char* text) {
    ...
    while (from < len) {
        ...
        while (from < len && (text[from] == ' ' || text[from] == '\n' ||
                              text[from] == '<' || text[from] == ',' ||
                              text[from] == '>' || text[from] == 'w')) {
            if (text[from] == 'w' && from + 5 < len &&
                strncmp(text + from, "where", 5) == 0) {
                from += 5;
            } else {
                from++;
            }
        ...
        if (text[from] == '\'') {
            /* Lifetime — skip. */
            from++;
            while (from < len && (isalnum((unsigned char)text[from]) ||
                                  text[from] == '_')) from++;
            continue;
        }
        while (from < len && (isalnum((unsigned char)text[from]) ||
        ...
        char* pname = cbm_arena_strndup(ctx->arena,
            text + name_start, (size_t)(name_end - name_start));
        /* Look for `:`. */
        while (from < len && (text[from] == ' ' || text[from] == '\t')) from++;
        ...
        while (from < len) {
            ...
            if (c == '<' || c == '(' || c == '[') depth++;
                ...
                int b_end = from;
                /* Trim trailing whitespace. */
                while (b_end > bound_start &&
                       (text[b_end - 1] == ' ' || text[b_end - 1] == '\t')) {
                    b_end--;
                }
                /* Trim leading whitespace. */
                int b_start = bound_start;
                while (b_start < b_end &&
                       (text[b_start] == ' ' || text[b_start] == '\t')) {
                    b_start++;
                }
                if (b_end > b_start) {
                    char* btext = cbm_arena_strndup(ctx->arena,
                        text + b_start, (size_t)(b_end - b_start));
...
static bool rust_impl_has_type_param(RustLSPContext* ctx, TSNode impl_node,
    const char* name) {
    if (!name) return false;
    TSNode tp = ts_node_child_by_field_name(impl_node, "type_parameters", 15);
    if (ts_node_is_null(tp)) return false;
    uint32_t nc = ts_node_named_child_count(tp);
    for (uint32_t i = 0; i < nc; i++) {
        TSNode c = ts_node_named_child(tp, i);
        const char* ck = ts_node_type(c);
        if (strcmp(ck, "type_identifier") == 0 ||
            strcmp(ck, "constrained_type_parameter") == 0) {
            ...
            if (strcmp(ck, "constrained_type_parameter") == 0) {
                ...
                if (!ts_node_is_null(lhs)) nm = rust_node_text(ctx, lhs);
            }
...
static void rust_process_impl(RustLSPContext* ctx, TSNode impl_node) {
    ...
    bool is_blanket = !ts_node_is_null(trait_node) &&
                      rust_impl_has_type_param(ctx, impl_node, type_text);
    const char* effective_recv = NULL;

    if (is_blanket) {
        char* tt = rust_node_text(ctx, trait_node);
        if (tt) effective_recv = rust_resolve_path_expr(ctx, tt);
    } else {
        effective_recv = rust_resolve_path_expr(ctx, type_text);
    }
    if (!effective_recv) return;
    ...
    if (!ts_node_is_null(trait_node) && !is_blanket) {
        char* tt = rust_node_text(ctx, trait_node);
        if (tt) ctx->self_trait_qn = rust_resolve_path_expr(ctx, tt);
    }
...
void rust_lsp_process_file(RustLSPContext* ctx, TSNode root) {
     ...
     * already in this file. Only bare `mod foo;` declarations are. */
    {
        ...
        for (uint32_t i = 0; i < rnc; i++) {
            TSNode c = ts_node_child(root, i);
            if (ts_node_is_null(c)) continue;
            if (strcmp(ts_node_type(c), "mod_item") != 0) continue;
            /* Inline mod has a `body` field; bare decl does not. */
            TSNode body = ts_node_child_by_field_name(c, "body", 4);
            if (!ts_node_is_null(body)) continue;  /* inline */
        ...
        if (strcmp(ck, "function_item") == 0) {
            ...
            if (!ts_node_is_null(body)) {
                ...
                for (uint32_t j = 0; j < mnc; j++) {
                    ...
                    if (ts_node_is_null(mc)) continue;
...
static void rust_collect_uses(RustLSPContext* ctx, TSNode root) {
    /* Recursive walker. */
    typedef struct stack_t { TSNode node; struct stack_t* prev; } stack_t;
            ...
            if (full) {
                ...
                if (flen >= 3 && strcmp(full + flen - 3, "::*") == 0) {
                    ...
                    if (lbr) {
                        ...
                        if (blen > 0 && body[blen - 1] == '}') body[blen - 1] = '\0';
                        char* save = NULL;
                        char* tok = strtok_r(body, ",", &save);
                        while (tok) {
                            while (*tok == ' ') tok++;
                            char* eb = tok + strlen(tok) - 1;
                            while (eb > tok && *eb == ' ') *eb-- = '\0';
                            if (*tok == '\0') {
                            ...
                            char* full_path = (strcmp(tok, "self") == 0)
...
static void rust_build_registry_from_defs(CBMArena* arena, CBMTypeRegistry* reg,
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        if (d->label && (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0)) {
            ...
            if (d->return_types) {
                ...
                if (count > 0) {
                    ret_types = (const CBMType**)cbm_arena_alloc(arena,
                        (count + 1) * sizeof(const CBMType*));
                    for (int j = 0; j < count; j++) {
                        ret_types[j] = rust_parse_return_type_text(arena, d->return_types[j], module_qn);
                    }
            ...
            if (d->param_types) {
                ...
                if (count > 0) {
                    param_types = (const CBMType**)cbm_arena_alloc(arena,
                        (count + 1) * sizeof(const CBMType*));
                    for (int j = 0; j < count; j++) {
                        param_types[j] = rust_parse_return_type_text(arena, d->param_types[j], module_qn);
                    }
    ...
    if (!ts_node_is_null(root)) {
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(tk, "trait_item") == 0) {
                ...
                for (int ti = 0; ti < reg->type_count; ti++) {
                    if (!reg->types[ti].qualified_name) continue;
    ...
    if (!ts_node_is_null(root)) {
        ...
        for (uint32_t i = 0; i < rnc; i++) {
            ...
            for (int k = 0; k < reg->func_count; k++) {
                ...
                if (!rf->qualified_name) continue;
                if (rf->receiver_type) continue;  /* free fns only */
                if (strcmp(rf->qualified_name, fn_qn) != 0) continue;
                const CBMType** ret_arr = (const CBMType**)cbm_arena_alloc(arena,
     ...
     */
    {
        ...
        static const struct DeriveImpl derives[] = {
            {"Clone",      "core.clone.Clone",
             {{"clone", NULL, false}, {NULL, NULL, false}}},
            {"Copy",       "core.marker.Copy",
             {{NULL, NULL, false}}},  /* marker — no methods */
            {"Debug",      "core.fmt.Debug",
             {{"fmt", NULL, false}, {NULL, NULL, false}}},
            {"Display",    "core.fmt.Display",
             {{"fmt", NULL, false}, {NULL, NULL, false}}},
            {"Default",    "core.default.Default",
             {{"default", NULL, true}, {NULL, NULL, false}}},
            {"PartialEq",  "core.cmp.PartialEq",
             {{"eq", "bool", false}, {"ne", "bool", false}, {NULL, NULL, false}}},
            {"Eq",         "core.cmp.Eq",
             {{NULL, NULL, false}}},  /* marker only */
            {"PartialOrd", "core.cmp.PartialOrd",
             {{"partial_cmp", NULL, false}, {"lt", "bool", false},
              {"le", "bool", false}, {NULL, NULL, false}}},
            {"Ord",        "core.cmp.Ord",
             {{"cmp", NULL, false}, {NULL, NULL, false}}},
            {"Hash",       "core.hash.Hash",
             {{"hash", "()", false}, {NULL, NULL, false}}},
            {"Send",       "core.marker.Send",
             {{NULL, NULL, false}}},
            {"Sync",       "core.marker.Sync",
             {{NULL, NULL, false}}},
            ...
            {"Serialize",  "serde.Serialize",
             {{"serialize", NULL, false}, {NULL, NULL, false}}},
            {"Deserialize","serde.Deserialize",
             {{"deserialize", NULL, true}, {NULL, NULL, false}}},
            /* clap derive — synthesizes the Parser interface. */
            {"Parser",     "clap.Parser",
             {{"parse", NULL, true}, {"try_parse", NULL, true},
              {"parse_from", NULL, true}, {"try_parse_from", NULL, true}}},
            {"Args",       "clap.Args",
             {{NULL, NULL, false}}},
        ...
        const int derive_count =
            (int)(sizeof(derives) / sizeof(derives[0]));

        for (int i = 0; i < result->defs.count; i++) {
            CBMDefinition* d = &result->defs.items[i];
            if (!d->qualified_name || !d->name) continue;
            if (!d->label || (strcmp(d->label, "Class") != 0 &&
                              strcmp(d->label, "Type") != 0)) continue;
            if (!d->decorators) continue;
            ...
            for (int di = 0; d->decorators[di]; di++) {
                ...
                while (q < rparen) {
                    ...
                    for (int di2 = 0; di2 < derive_count; di2++) {
                        ...
                        if (strncmp(di_entry->derive_name, short_start, name_len) != 0) continue;
                        ...
                        for (int ti = 0; ti < reg->type_count; ti++) {
                            if (reg->types[ti].qualified_name &&
                                strcmp(reg->types[ti].qualified_name,
                                       d->qualified_name) == 0) {
                                rt = &reg->types[ti];
                                break;
                            }
                        ...
                        int existing = 0;
                        if (rt->embedded_types) {
                            while (rt->embedded_types[existing]) existing++;
                        }
                        const char** new_arr = (const char**)cbm_arena_alloc(arena,
                            (existing + 2) * sizeof(const char*));
                        ...
                        for (int mi = 0;
                            ...
                            rf.qualified_name = cbm_arena_sprintf(arena, "%s.%s",
                                d->qualified_name, dm->short_name);
    ...
    if (!ts_node_is_null(root)) {
        ...
        for (uint32_t i = 0; i < rnc; i++) {
            TSNode top = ts_node_child(root, i);
            if (ts_node_is_null(top) || strcmp(ts_node_type(top), "impl_item") != 0) continue;
            TSNode type_node = ts_node_child_by_field_name(top, "type", 4);
            TSNode body = ts_node_child_by_field_name(top, "body", 4);
            if (ts_node_is_null(type_node) || ts_node_is_null(body)) continue;
            ...
            for (uint32_t j = 0; j < bnc; j++) {
                ...
                if (ts_node_is_null(item) || !ts_node_is_named(item)) continue;
                if (strcmp(ts_node_type(item), "function_item") != 0) continue;
                TSNode mn = ts_node_child_by_field_name(item, "name", 4);
                ...
                for (int k = 0; k < reg->func_count; k++) {
                    ...
                    if (strcmp(rf->receiver_type, type_qn) != 0) continue;
                    if (strcmp(rf->short_name, mname) != 0) continue;
                    const CBMType** ret_arr = (const CBMType**)cbm_arena_alloc(arena,
                        2 * sizeof(const CBMType*));
                    ret_arr[0] = ret;
                    ret_arr[1] = NULL;
    ...
    for (int i = 0; i < result->impl_traits.count; i++) {
        ...
        if (!it->struct_name || !it->trait_name) continue;
        ...
        const char** new_arr = (const char**)cbm_arena_alloc(arena,
            (existing + 2) * sizeof(const char*));
        for (int j = 0; j < existing; j++) new_arr[j] = rt->embedded_types[j];
    ...
    for (int i = 0; i < def_count; i++) {
        ...
        if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
            ...
            if (rf.receiver_type && d->trait_qn && d->trait_qn[0]) {
                ...
                if (rt) {
                    int existing = 0;
                    if (rt->embedded_types) while (rt->embedded_types[existing]) existing++;
                    const char** new_arr = (const char**)cbm_arena_alloc(arena,
                        (existing + 2) * sizeof(const char*));
                    for (int j = 0; j < existing; j++) new_arr[j] = rt->embedded_types[j];
